        ...
        <div className="flex justify-center gap-6 mb-20 flex-wrap px-4">
          {['day1', 'day2', 'day3'].map((day, idx) => (
            <button
              ...
              className={`cursor-pointer group relative px-8 py-3 font-display font-black italic text-xl tracking-wider transition-all duration-300 border-4 border-black shadow-[6px_6px_0px_#000] ${
                activeDay === day
                  ? 'bg-[#10B981] text-white hover:translate-y-[-2px] hover:shadow-[8px_8px_0px_#000]'
                  : 'bg-white text-black hover:bg-emerald-100 hover:translate-y-[-2px] hover:shadow-[8px_8px_0px_#10B981]'
              }`}
            >
          ...
          {currentSchedule.map((item, index) => (
            <div key={`${activeDay}-${index}`} className={`relative flex items-center mb-16 ${index % 2 === 0 ? 'md:flex-row-reverse' : ''}`}>
              ...
              <div className="absolute left-[15px] sm:left-[19px] md:left-1/2 -translate-x-1/2 z-20 group">
                  <div className="w-10 h-10 sm:w-12 sm:h-12 bg-black border-4 border-[#10B981] rotate-45 group-hover:rotate-135 transition-transform duration-700 shadow-[4px_4px_0px_#000] flex items-center justify-center relative overflow-hidden">
                      <div className="absolute inset-0 bg-emerald-900/50"></div>
                      <div className="w-4 h-4 bg-white rounded-full border-2 border-black relative z-10 animate-pulse"></div>
                  </div>
              </div>
              ...
              <div className="ml-12 sm:ml-16 md:ml-0 md:w-1/2 md:px-12">
                <div 
                  className={`relative bg-white border-4 border-black p-6 sm:p-8 hover:translate-y-[-4px] transition-all duration-300 group ${index % 2 === 0 ? 'md:text-left shadow-[8px_8px_0px_#10B981]' : 'md:text-right shadow-[-8px_8px_0px_#10B981]'} hover:shadow-[12px_12px_0px_#000]`}
                ...
                >
                  ...
                  <div className="absolute -top-2 -left-2 w-0 h-0 border-t-10 border-l-10 border-[#10B981] z-20"></div>
                  <div className="absolute -bottom-2 -right-2 w-0 h-0 border-b-10 border-r-10 border-[#10B981] z-20"></div>

                  <div className={`flex flex-col ${index % 2 === 0 ? 'md:items-start' : 'md:items-end'}`}>
                      <div className="bg-black text-white px-2 py-1 font-mono font-bold text-xs inline-block mb-2 transform -rotate-2 border border-white">
                          TIMESTAMPS // {item.time}
                      </div>
                      <h3 className="relative z-10 text-2xl sm:text-3xl font-display font-black text-black mb-2 uppercase tracking-wide italic leading-none">
                          {item.title}
                      </h3>
                      <p className="relative z-10 text-black font-bold font-sans text-sm sm:text-base leading-relaxed max-w-sm">
                          {item.description}
                      </p>
                  </div>
                </div>
